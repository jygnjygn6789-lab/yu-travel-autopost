...
def _claude_generate(destination: str) -> dict:
    ...
    prompt = f"""你是台灣旅遊達人，請用你所有知識為「{destination}」生成超實用旅遊懶人包。

重要原則：每一條都要有「具體數字/品牌/地名」，不可以說廢話或只說標題。

❌ 不好的例子：「提早訂機票比較便宜」、「住在市中心最方便」、「注意飲食衛生」
✅ 好的例子：「提前 45 天訂，長榮直飛約 NT$8,500」、「新宿站步行 10 分鐘的 APA Hotel 每晚 NT$2,500」、「地鐵 IC 卡 PASMO 買一次用到底，省換票時間」

請以 JSON 格式回應，每主題 4 條，每條最多 30 字：
{{
  "機票": ["條1", "條2", "條3", "條4"],
  "住宿": ["條1", "條2", "條3", "條4"],
  "交通": ["條1", "條2", "條3", "條4"],
...
各主題要求（繁體中文，台灣人視角）：
- 機票：訂票時機（提前幾週最便宜）、推薦航空+有無直飛、來回大約 NT$ 多少、廉航選項
- 住宿：推薦住哪個區+理由、具體旅館品牌或名稱、每晚大約 NT$ 多少、訂房平台推薦
- 交通：機場到市區怎麼搭（交通工具+票價+時間）、市內交通票卡名稱+費用、計程車/共享叫車 App 名稱
- 美食：【格式】「店名（原文）｜區域｜必點：菜名」，4 家真實知名店家，要有正確店名
- eSIM：推薦品牌（Airalo/Klook eSIM/數位旅遊等）、幾天方案多少 NT$、速度夠不夠用、購買方式
- 注意事項：當地詐騙手法或踩雷、禁忌或法規（不遵守罰款）、匯率換算（1 JPY/KRW/THB ≈ NT$多少）、季節或疫苗注意"""
